...
epsilon = 0.0045; %0.001 - 0.0045 is good for f1
i = 0 ; 
...
for i = 1:10 %update epsilon 9 times(run for 10 different epsilon values)
    ...
    epsilon = epsilon - 0.00035; %update value of epsilon
    fprintf('i:%d  e:%f\n', i, epsilon);
    while d > l %continue as long as the width is not limited enough
        fx1 = subs({f1}, {x}, {x1}); %calculate f(x1)
        fx2 = subs({f1}, {x}, {x2}); %calculate f(x2)
    ...
    plot(epsilon,k, 'r.'); %Horizontal axis = epsilon values....Vertical axis = k values
    hold on;
    fprintf("\n");
end

%set x and y axis limits for better representation 
xlim([0.5*10^(-3) 4.5*10^(-3)]); %set figure limit of x Axis 
ylim([11 16]); %set figure limit of y Axis

xlabel('epsilon values');
ylabel('iterations needed to finish the algorithm');

title('f1(x) = (x-3)^2 + (sin(x+3))^2');
grid on;
