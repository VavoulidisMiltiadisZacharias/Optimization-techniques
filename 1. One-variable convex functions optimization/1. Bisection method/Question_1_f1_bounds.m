...
for i = 1:10 %update lamda 9 times(run for 10 different lamda values)
    ...
    while d > l %continue as long as the width is not limited enough
        ...
        %The more left the less difference!!!(but k increases -> more iterations needed)
        plot(k, A, 'r.');
        hold on;
        plot(k, B, 'b.');
    end
    %plot3(l,A,B, 'r.'); %X axis = lambda .... Y axis = A ..... Z axis = B
    
    fprintf("\n");
end

%set x and y axis limits for better representation 
xlim([0 12]); %set figure limit of x Axis 
